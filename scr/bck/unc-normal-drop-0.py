...
### important features
# optional_drop_features = ['CNT', 'ontology_1', 'DP', 'AF', 'AD', 'ontology_3']

### useless features
optional_drop_features = ['trinucleotide_22']
...
def accuracy(model, test_val, preds):
    ...
    plt.savefig(output_dir + "/roc-" + model + ".png")
    plt.show()
...
def myplot_importance(feature_names, feature_importance, model_name,
    ...
    plt.tight_layout()
    plt.savefig(output_dir + '/importance-' + model_name + '.png')
    plt.show()
...
if optional_drop_features == ['none']:
    drop_cols = ['subtype', 'fastq id', 'ANN',
             '#CHROM', 'POS']
else:
    drop_cols = ['subtype', 'fastq id', 'ANN',
             '#CHROM', 'POS'] + optional_drop_features
...
### make predictions
preds_xgboost = bst.predict(validation_X.drop(columns=drop_cols))
### accuracy check
accuracy('xgboost', validation_Y, preds_xgboost)

### train
preds_xgboost = bst.predict(train_X.drop(columns=drop_cols))
...
### test melanoma
preds_xgboost = bst.predict(test_X_m.drop(columns=drop_cols))
...
### test mixtcga
preds_xgboost = bst.predict(test_X_mix.drop(columns=drop_cols))
...
### test tnbc data
preds_xgboost = bst.predict(test_X.drop(columns=drop_cols))
